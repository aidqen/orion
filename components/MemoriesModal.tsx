...
export function MemoriesModal({ open, onOpenChange }: MemoriesModalProps) {
    ...
    const { data: memories, isLoading } = useQuery({
        ...
        select: (memoriesData) => memoriesData.filter(memory =>
        ...
        >
            <div className="pt-[20px] px-[24px] pb-0 flex flex-col gap-[8px] shrink-0">
                <div className="flex justify-between items-start">
                    <h2 className="text-2xl font-medium text-gray-900 dark:text-white m-0">Saved memories</h2>
                    <button
                ...
                <p className="text-[13px] text-gray-500 dark:text-[#B4B4B4] leading-[1.4]">
                    Planwise tries to remember your recent chats, but it may forget things over time. Saved memories are never forgotten.{" "}
                    <a href="#" className="underline hover:text-gray-900 dark:hover:text-white transition-colors">Learn more</a>
                </p>
            </div>
            ...
            <div className="px-[24px] pb-[24px] overflow-y-auto grow flex flex-col custom-scrollbar">
                {isLoading ? (
                    <>
                        {[...Array(4)].map((_, i) => (
                            <div key={i} className="py-[16px] border-b border-gray-200 dark:border-[#333333] last:border-b-0">
