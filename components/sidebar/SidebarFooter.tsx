...
export function SidebarFooter({ }: SidebarFooterProps) {
    const { user } = useUser()
    return (
        <ShadcnSidebarFooter className="mt-auto grid grid-cols-[auto_1fr_auto] gap-3 items-center hover:bg-gray-100 transition-colors cursor-pointer pb-3">
            ...
            <div className="flex flex-col gap-0">
                <span className="text-black dark:text-white text-sm font-medium">{user?.user_metadata?.name || "User"}</span>
                <span className="text-stone-600 dark:text-stone-400 text-xs">Free Plan</span>
            </div>
            <div className="flex flex-col gap-0">
                <ChevronUp size={10} />
                <ChevronDown size={10} />
