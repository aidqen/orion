...
const Title: React.FC<TitleProps> = ({ children, className }) => (
	<h3
		className={`dark:text-white text-black font-semibold leading-none text-sm sm:text-base tracking-tight truncate ${className || ""}`}
	>
		{children}
	</h3>
);
...
const Description: React.FC<DescriptionProps> = ({ children, className }) => (
	<p
		className={`text-xs sm:text-sm text-muted-foreground truncate ${className || ""}`}
	>
		{children}
	</p>
);
