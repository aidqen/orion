...
export async function fetchCalendarEvents(userId: string, minDate: string, maxDate: string): Promise<CalendarEvent[]> {
    ...
    const calendarIds = (data.items || []).map(cal => cal.id).filter(Boolean) as string[];

    // Fetch events from all calendars in parallel
    const eventsArrays = await Promise.all(
        calendarIds.map(id => fetchEventsFromCalendar(calendar, id, timeMin, timeMax))
    );
